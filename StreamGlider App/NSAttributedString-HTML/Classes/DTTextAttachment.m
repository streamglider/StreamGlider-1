...
- (id)contents
{
	if (!contents)
	{
		if (contentType == DTTextAttachmentTypeImage && _contentURL)
		{
            UIImage *image;
            if ([_contentURL isFileURL]) {
                image = [UIImage imageWithContentsOfFile:[_contentURL path]];
            } else {
                image = [[[UIImage alloc] initWithData:[NSData dataWithContentsOfURL:_contentURL]] autorelease]; //GMcK
            }
			return image;
		}
	}
