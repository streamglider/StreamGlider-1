//
//  NewYTSourceTableViewController.m
//  StreamCast
//
//  Created by Dmitry Shingarev on 22/09/2010.
...
@implementation NewYTSourceTableViewController {
    BOOL isUserSelected;	
	BOOL titleWasEdited;
...
static NSString *UserURL = @"http://gdata.youtube.com/feeds/api/users/";
static NSString *SearchURL = @"http://gdata.youtube.com/feeds/api/videos?q=";
...
- (void)handleDoneButtonTapped {
	...
	if (isUserSelected) {
		feedSource.URLString = [NSString stringWithFormat:@"%@%@/uploads", UserURL, queryString];
	} else {
        ...
        if (locationEnabled) {
            ...
            if (loc != nil) {
                feedSource.URLString = [NSString stringWithFormat:@"%@&location=%.5f,%.5f&location-radius=10km", feedSource.URLString, loc.coordinate.latitude, loc.coordinate.longitude];                
            } else {
...
- (void)prepopulateFields {
	if (feed != nil && feed.source.type == FeedSourceTypeYouTube) {
		// pre populate fields with data from the source
		self.titleString = feed.source.title;
        
		isUserSelected = [feed.source.URLString rangeOfString:@"/users/"].location != NSNotFound;        
        locationEnabled = [feed.source.URLString rangeOfString:@"location="].location != NSNotFound;
        
		NSString *searchStr;
		searchStr = isUserSelected ? @"users/" : @"videos?q=";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.section == 0) {
		...
		if (cell == nil) {
			...
			cell.imageName = @"Youtube_Panel_Borderless.png";	
		}
        ...
        if (indexPath.row == 2) {
            ...
            cell.textLabel.text = @"Search for videos near you";
            if (locationEnabled) 
...
- (void)viewDidLoad {
	...
	self.title = @"YouTube Feed";
	
	self.feedSource = [[[FeedSource alloc] init] autorelease];
	feedSource.type = FeedSourceTypeYouTube;
