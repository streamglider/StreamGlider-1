...
- (NSComparisonResult)scoreComparison:(HTMLNode *)otherNode
{
    NSComparisonResult result = NSOrderedSame;
    
    if (self.score < otherNode.score) {
        result = NSOrderedAscending;
    } else if (self.score > otherNode.score) {
...
-(HTMLNode*)parentWithTagNameInSet:(NSArray *)tagNames
{
    if (!_node || !tagNames)
        return nil;

    HTMLNode *result = nil;
    HTMLNode *parentNode = [self parent];
    
    while (parentNode) {
        NSString *parentTagName = parentNode.tagName;
        if (parentTagName == nil || [parentTagName length] == 0) {
            parentNode = nil;
            break;
        }
        if ([tagNames containsObject:parentTagName]) {
            break;
        } else {
...
-(HTMLNode*)parent
{
    HTMLNode *result = nil;
    if (_node) {
        result = [[[HTMLNode alloc] initWithXMLNode:_node->parent] autorelease];
    }
    return result;
}
...
-(HTMLNode*)nextSibling {
    HTMLNode *result = nil;
    if (_node) {
        result = [[[HTMLNode alloc] initWithXMLNode:_node->next] autorelease];
    }
    return result;
}
...
-(HTMLNode*)previousSibling {
    HTMLNode *result = nil;
    if (_node) {
        result = [[[HTMLNode alloc] initWithXMLNode:_node->prev] autorelease];
    }
    return result;
}
...
void setAttributeNamed(xmlNode * node, const char * nameStr, const char * value) {
	...
	{
		if (strcasecmp((char*)attr->name, nameStr) == 0)
		{				
...
-(void)findChildrenWithAttribute:(const char*)attribute matchingName:(const char*)className inXMLNode:(xmlNode *)node inArray:(NSMutableArray*)array allowPartial:(BOOL)partial
{
	...
	{				
		...
		{
			if (strcasecmp((char*)attr->name, attribute) == 0)
			{				
				for(xmlNode * child = attr->children; NULL != child; child = child->next)
				{
					
					BOOL match = NO;
					if (!partial && strcasecmp((char*)child->content, classNameStr) == 0)
						match = YES;
...
-(void)findChildTags:(NSString*)tagName inXMLNode:(xmlNode *)node inArray:(NSMutableArray*)array
{
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next) 
	{				
		if (cur_node->name && strcasecmp((char*)cur_node->name, tagNameStr) == 0)
		{
...
-(HTMLNode*)findChildTag:(NSString*)tagName inXMLNode:(xmlNode *)node
{
    ...
    for (cur_node = node; cur_node; cur_node = cur_node->next) 
	{				
		if (cur_node && cur_node->name && strcasecmp((char*)cur_node->name, tagNameStr) == 0)
		{
...
HTMLNodeType nodeType(xmlNode * _node)
{
	...
	if (strcasecmp(tagName, "a") == 0)
		return HTMLHrefNode;
	else if (strcasecmp(tagName, "text") == 0)
		return HTMLTextNode;
	else if (strcasecmp(tagName, "code") == 0)
		return HTMLCodeNode;
	else if (strcasecmp(tagName, "span") == 0)
		return HTMLSpanNode;
	else if (strcasecmp(tagName, "p") == 0)
		return HTMLPNode;
	else if (strcasecmp(tagName, "div") == 0)
		return HTMLDivNode;
	else if (strcasecmp(tagName, "ul") == 0)
		return HTMLUlNode;
	else if (strcasecmp(tagName, "li") == 0)
		return HTMLLiNode;
	else if (strcasecmp(tagName, "image") == 0)
		return HTMLImageNode;
	else if (strcasecmp(tagName, "ol") == 0)
		return HTMLOlNode;
	else if (strcasecmp(tagName, "strong") == 0)
		return HTMLStrongNode;
	else if (strcasecmp(tagName, "pre") == 0)
		return HTMLPreNode;
	else if (strcasecmp(tagName, "blockquote") == 0)
